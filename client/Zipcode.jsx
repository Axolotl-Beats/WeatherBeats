...
export default function Zipcode() {
    ...
    useEffect(() => {
        ...
        const { type, temp, city, url } = getWeatherData(location); //Uncomment this line when we are ready to get 
    ...
    async function getWeatherData(input) {
        await fetch(weatherAPI, {
            ...
            .then(response => { return response.body }); //we may have to access a part of the response object 
    }
    ...
    return (
        <div className="box has-background-danger">
            <div>
                <input type="text" placeholder="zipcode" onChange={(e) => setLocation(e.target.value)}></input>
                <button onClick={() => setClicked(!clicked)}>Update Location</button>
            </div>
            <br></br>
            <div className="infoBar">{temp}{city}{type}</div>
        </div>
    )
}
